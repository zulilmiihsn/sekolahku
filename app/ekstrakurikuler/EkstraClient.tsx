...
export default function EkstraClient({ ekstrakurikuler }: { ekstrakurikuler: EkstraItem[] }) {
  ...
  return (
    <>
      {ekstrakurikuler.length === 0 ? (
        <div className="text-center text-text/60 py-16">Belum ada ekstrakurikuler.</div>
      ) : (
            ...
            {ekstrakurikuler.map((item) => (
              <div key={item.id} className="bg-background rounded-2xl shadow-lg p-8 flex flex-col gap-4">
                ...
                {item.foto && item.foto.length > 0 ? (
                  <EkstraSlider foto={item.foto} onClick={idx => openGaleri(item.foto, idx)} />
                ) : (
                  <NoPhotoPlaceholder />
                )}
              </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-4 md:p-8 flex flex-col items-center max-w-2xl w-full">
            ...
            <div className="flex items-center gap-4 w-full">
              ...
              <Image
                ...
                alt={`Galeri Ekstrakurikuler ${galeriIdx+1}`}
                width={900}
                height={600}
...
function EkstraSlider({ foto, onClick }: { foto: string[], onClick: (idx: number) => void }) {
      ...
      >
        {foto.map((url, i) => (
          <div
            ...
            onClick={() => onClick(i)}
          >
            <Image src={url} alt={`Foto ekstrakurikuler ${i+1}`} width={600} height={340} sizes="(max-width: 768px) 100vw, 600px" className="object-cover w-full h-full" />
          </div>
        ))}
      </div>
      {foto.length > 1 && (
